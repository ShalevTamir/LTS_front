...
export class LiveParametersSocketService {
  ...
  public async addParameter(parameterName: string){
    ...
    await firstValueFrom(this._httpClient.post(LIVE_DATA_URL+"/socket-config/add-parameter", parameterToAdd));
  }
  ...
  public async removeParameter(parameterName: string){
    ...
    await firstValueFrom(this._httpClient.post(LIVE_DATA_URL+"/socket-config/remove-parameter", parameterToRemove));
  }
